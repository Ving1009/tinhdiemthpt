...
test("chuyển sang Tesseract khi Gemini hết hạn mức", async () => {
  const calls = [];
  const scan = createTranscriptScanService([
    ...
    { name: "tesseract", label: "OCR Tesseract cục bộ", scan: async () => { calls.push("tesseract"); return { data: { scores: [] }, warnings: ["Kiểm tra lại."] }; } }
  ]);
  const result = await scan([]);
  assert.deepEqual(calls, ["gemini", "tesseract"]);
  assert.equal(result.engine, "tesseract");
  assert.match(result.warnings[0], /Đã chuyển sang OCR Tesseract cục bộ/);
});

test("ưu tiên OCR.space trước Tesseract khi Gemini không dùng được", async () => {
  const calls = [];
  const scan = createTranscriptScanService([
    { name: "gemini", label: "Gemini", scan: async () => { calls.push("gemini"); throw new AppError("limit", { statusCode: 503, code: "AI_QUOTA" }); } },
    { name: "ocr-space", label: "OCR.space", scan: async () => { calls.push("ocr-space"); return { data: { scores: [] }, warnings: [] }; } },
    { name: "tesseract", label: "OCR Tesseract cục bộ", scan: async () => { calls.push("tesseract"); return { data: { scores: [] }, warnings: [] }; } }
  ]);
  const result = await scan([]);
