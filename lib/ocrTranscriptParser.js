...
function layoutFrom(text) {
  ...
  return {
    ...
    year: /\b(?:ca nam|tbcn|tbm\s*cn|trung binh nam)\b/.test(value),
...
export function parseOcrTranscriptPages(pages, catalog) {
  ...
  for (const page of pages) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      let scoreText = match.normalized.slice(match.index + match.alias.length)
      ...
      if (!currentGrade) {
        warnings.push(`${match.subject}: OCR chưa xác định được lớp nên chưa tự điền.`);
        continue;
      }
  ...
  if (!scores.length && recognizedSubjectLines) warnings.push("OCR đọc được tên môn nhưng chưa xác định chắc lớp và cột điểm.");
